...
const EpubViewerScreen = ({ route }) => {
  ...
  const downloadEpub = async () => {
    try {
      ...
      const fileName = bookurl.split('/').pop(); // Extract file name from URL
  ...
  return (
    <SafeAreaView style={backgroundStyle}>
      ...
      <View style={styles.topBar}>
        ...
        <TouchableOpacity onPress={() => EPubReaderModule.highlightSelection()} style={styles.iconButton}>
          <Ionicons name="pencil-outline" size={24} color={isDarkMode ? '#fff' : '#000'} />
        </TouchableOpacity>

        <TouchableOpacity onPress={() => setPopupVisible(true)} style={styles.dotButton}>
      ...
      {isDownloading && (
        <View style={styles.loadingContainer}>
          <Text style={[styles.loadingText, { color: isDarkMode ? '#fff' : '#000' }]}>Downloading book...</Text>
        </View>
      )}
            ...
            <TouchableWithoutFeedback>
              <View style={styles.popup}>
                ...
                >
                  ...
                  <Entypo name={fontDropdownVisible ? 'chevron-down' : 'chevron-right'} size={20} color="#000" style={styles.chevronIcon} />
                </TouchableOpacity>

                {fontDropdownVisible && (
                  <View>
                    {fonts.map((font) => (
                      <TouchableOpacity key={font} onPress={() => {
                        setFontFamily(font);
                        setFontDropdownVisible(false);
                      }} style={styles.fontOption}>
                        <Text style={styles.fontOptionText}>{font.charAt(0).toUpperCase() + font.slice(1)}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                )}
                ...
                <View style={styles.themeRow}>
                  <TouchableOpacity
                    style={[styles.themeButton, { backgroundColor: '#fff' }, !isDarkMode && styles.activeThemeBorder]}
                  ...
                  <TouchableOpacity
                    style={[styles.themeButton, { backgroundColor: '#000' }, isDarkMode && styles.activeThemeBorder]}
