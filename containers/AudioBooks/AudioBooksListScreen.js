...
const AudioBooksListScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    setBookData([]); // Reset data when genre changes
    setPage(1);
    setHasMore(true);
  }, [genre]);
  ...
  const loadBooks = async () => {
    ...
    try {
      const response = await fetch(
        `https://freeinvoicegenerator.pk/api/audiobooks/genres/${genre}?page=${page}&limit=${limit}`
      );
      ...
      const books = data.map(book => ({
        ...
        audiolinks: book.audiolinks,
      }));

      setBookData(prev => (page === 1 ? books : [...prev, ...books]));
  ...
  const handleDownloadPress = async (book) => {
    ...
    try {
      ...
      const exists = existingBooks.some(b => b.bookid === bookToSave.bookid);
      if (exists) {
        Alert.alert('Notice', 'This book is already in the Library');
      } else {
        const updatedBooks = [...existingBooks, bookToSave];
...
>
  <Text style={styles.buttonText}>Books</Text>
              ...
              <TouchableOpacity style={[styles.button1, { backgroundColor: '#E04B07' }]} onPress={() => navigation.navigate('AudioBooksScreen')}>
